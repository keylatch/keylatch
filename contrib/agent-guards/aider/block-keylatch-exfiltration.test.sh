#!/usr/bin/env bash
# Test harness for the Aider block-keylatch-exfiltration.sh guard.
#
...
run_case() {
	...
	actual_code=0
	AIDER_TOOL_NAME="$tool_name" AIDER_TOOL_INPUT="$tool_input" bash "$HOOK" </dev/null 2>/dev/null || actual_code=$?
...
# standalone).
INTERNAL_HOOK="$SCRIPT_DIR/../../../internal/guard/scripts/aider-guard.sh"
...
echo
echo "Aider guard: $PASS passed, $FAIL failed"
[ "$FAIL" -eq 0 ]
