...
# Aider agent guard — blocks credential-access patterns before tool calls execute.
# Invoked by Aider's pre-tool-use-hook configuration in ~/.aider.conf.yml.
set -euo pipefail

TOOL_NAME="${AIDER_TOOL_NAME:-}"
TOOL_INPUT="${AIDER_TOOL_INPUT:-}"
