#!/usr/bin/env bash
# Test harness for the GitHub Copilot CLI block-keylatch-exfiltration.sh guard.
#
...
run_case() {
	...
	actual_code=0
	COPILOT_TOOL_NAME="$tool_name" COPILOT_TOOL_INPUT="$tool_input" bash "$HOOK" </dev/null 2>/dev/null || actual_code=$?
...
run_case ".keylatch read_file blocked"          read_file "~/.keylatch/vault/imports/plaintext.env"          2
...
# standalone).
INTERNAL_HOOK="$SCRIPT_DIR/../../../internal/guard/scripts/copilot-guard.sh"
...
echo
echo "Copilot guard: $PASS passed, $FAIL failed"
[ "$FAIL" -eq 0 ]
