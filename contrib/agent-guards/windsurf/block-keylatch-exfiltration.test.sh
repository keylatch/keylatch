#!/usr/bin/env bash
# Test harness for the Windsurf block-keylatch-exfiltration.sh guard.
#
...
# the full design rationale this variant now shares. (Windsurf has no
# Read-equivalent tool case in this guard, so no read-path tests here.)
set -uo pipefail
...
run_case() {
	...
	actual_code=0
	WINDSURF_TOOL_NAME="$tool_name" WINDSURF_TOOL_INPUT="$tool_input" bash "$HOOK" </dev/null 2>/dev/null || actual_code=$?
...
# standalone).
INTERNAL_HOOK="$SCRIPT_DIR/../../../internal/guard/scripts/windsurf-guard.sh"
...
echo
echo "Windsurf guard: $PASS passed, $FAIL failed"
[ "$FAIL" -eq 0 ]
