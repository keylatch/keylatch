...
# Cursor agent guard — blocks credential-access patterns before tool calls execute.
# Mirrors the claude-code guard; adjust TOOL_NAME/TOOL_INPUT env vars if Cursor
# uses different names in future hook API versions.
set -euo pipefail

TOOL_NAME="${CURSOR_TOOL_NAME:-${CLAUDE_TOOL_NAME:-}}"
TOOL_INPUT="${CURSOR_TOOL_INPUT:-${CLAUDE_TOOL_INPUT:-}}"
