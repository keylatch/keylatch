...
| `KEYLATCH_SESSION_TOKEN` | **Child-env only** — scoped Keylatch session JWT minted per exec, injected into the child process by `proxy.EnvInject` (`internal/proxy/env_proxy.go`) so the child's outbound HTTP calls can authenticate to the MITM proxy | Yes | gateway_proxy only (re-injected) |
...
| ~~`KEYLATCH_DAEMON_STATE_PATH`~~ | **Deprecated — no reader.** `paths.DaemonState()` still defines this override but has no remaining callers after the daemon lifecycle refactor. Do not rely on it; it will be removed in a future release. | No | Never |
