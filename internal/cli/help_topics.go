...
func newHelpTopicBackendsCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(c *cobra.Command, _ []string) error {
			...
			fmt.Fprintln(w, "Proton Pass\tproton-pass\tAll\tYes\tNo\tYes\tNo")
			...
			fmt.Fprintln(w, "AWS Secrets Manager\taws-sm\tAll\tYes (cloud)\tNo\tYes\tNo")
			fmt.Fprintln(w, "1Password Connect\top-connect\tAll\tYes\tNo\tYes\tNo")
			fmt.Fprintln(w, "GCP Secret Manager\tgcp-sm\tAll\tYes (cloud)\tNo\tYes\tNo")
			fmt.Fprintln(w, "Azure Key Vault\tazure-kv\tAll\tYes (cloud)\tNo\tYes\tNo")
