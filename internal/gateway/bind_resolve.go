...
// EnvListenKey is the environment variable that permits the gateway server to
...
const EnvListenKey = "KEYLATCH_GATEWAY_LISTEN"

// ResolveBindAddr computes the gateway server's bind address and whether a
// non-loopback bind should be explicitly permitted (ServerOptions.AllowExternalBind).
//
// Precedence (highest first):
//  1. listenFlag (--listen)
//  2. KEYLATCH_GATEWAY_LISTEN env var
